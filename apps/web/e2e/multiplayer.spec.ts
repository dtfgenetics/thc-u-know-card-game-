...
const colorChoiceKinds = new Set(['strain-switch', 'hotbox-plus-four', 'dealer-choice']);
...
async function chooseLegalCard(page: Page, coverage: Coverage): Promise<CardView | undefined> {
  ...
  const legal = hand.filter(card => {
    if (pendingDraw > 0) {
      if (card.kind === 'tolerance-break') return true;
      if (!drawKinds.has(card.kind)) return false;
    }
